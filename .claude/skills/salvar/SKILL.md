...
name: salvar
description: >
  Salva o trabalho do JemacPRO no GitHub (commit + push). Na primeira vez configura o repositório
...
1. Perguntar:
   ...
   > 2. Não, vou criar agora — me dá um nome pro repositório (ex: meu-jemacpro)"

2. **Se opção 1:** rodar `git init`, `git add .`, `git commit -m "Setup inicial do JemacPRO"`, `git branch -M main`, `git remote add origin <URL>`, `git push -u origin main`.
